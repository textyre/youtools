...
server.registerTool(
  ...
  {
    description:
      'Get video subtitles with a summarization prompt. Auto-detects video language if lang is not specified. ' +
      'Returns deduplicated plain text (not raw SRT) to keep output compact. ' +
    ...
    inputSchema: {
      ...
      lang: z
        .string()
        .optional()
        .describe('Subtitle language code (e.g. "ru", "en"). Auto-detected from video if omitted.'),
      format: z
        .string()
        .optional()
  ...
  async ({ videoId, lang, format }) => {
    const config = loadConfig()
    const poToken = config.subtitles.poToken || undefined

    // Auto-detect language if not specified
    let resolvedLang = lang ?? ''
    let autoGenerated = config.subtitles.preferAutoGenerated
    if (!resolvedLang) {
      const tracks = await listLanguages(videoId, poToken)
      // Prefer original track (e.g. "ru-orig")
      const origTrack = tracks.find((t) => t.lang.endsWith('-orig'))
      if (origTrack) {
        resolvedLang = origTrack.lang.replace('-orig', '')
        autoGenerated = true
      } else {
        // Prefer non-auto-generated track
        const manual = tracks.find((t) => !t.isAutoGenerated)
        if (manual) {
          resolvedLang = manual.lang
          autoGenerated = false
        } else if (tracks.length > 0) {
          resolvedLang = tracks[0].lang
          autoGenerated = true
        } else {
          resolvedLang = config.subtitles.defaultLanguage
        }
      }
    }
    ...
    // Convert SRT to compact plain text (no timestamps, deduplicated)
    const entries = parseSrt(srtContent)
    const plainText = srtToPlainText(entries)
